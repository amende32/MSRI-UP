...
%			See the course syllabus for more information -- it's available on \href{https://learn.uwaterloo.ca/}{Waterloo LEARN}.
		...
		Ideally, Agebraic Geometry occurs over rings other than $\mathbb{C}$. Let's see some examples of polynomial solving.
			...
			The security of the R.S.A Cryptosystem is based on this \emph{hardness}.
			\end{ex}
			\begin{ex}
			...
			\end{align*}
			\end{ex}
			Differential equations govern chemical reaction rates. So, when the reaction settles down the concentrations reactions an \emph{equillibrium}, and the concentrations wind up being \emph{real} solutions to a system of polynomial equations.
			\begin{problem}
	...
	\section{Introduction to Graph Theory} \lecture{February 15, 2013} % Today's notes were taken from Kevin Cho's handwritten notes (thanks!).
		...
		\begin{defn}
			The \textbf{complement} of the graph $G$, denoted $\overline G$, is the graph with $V(\overline G) = V(G)$ and edges $uv \in E(\overline G)$ if and only if $uv \not \in E(G)$.
		\end{defn}
		...
		\begin{defn}
			...
			\\ \\
			The neighbours of $v \in V$ are defined as $\set{u \in V | v u \in E}$.
		\end{defn}
		...
		\subsection{Walks, Cycles, and Paths}
			...
			\begin{proof}
				Among all paths, pick a path $P$ with as many edges as possible. We have:
				\begin{align*}
		...
		\begin{defn}
			The \textbf{girth} of a graph $G$, denoted $g(G)$, is the length of the shortest cycle in $G$. If $G$ has no cycles, then $g(G)$ is infinite.
		\end{defn}
		...
		\subsubsection{Hamiltonian Cycles}
			...
			\begin{ex}
				Here's another example of a graph with a Hamiltonian cycle.
				\begin{center}
					\begin{tikzpicture}%[scale=1.5]
						\node [dot={0}{}] at (0,4) {};
						\node [dot={0}{}] at (1,0) {};
...
;					\end{tikzpicture}
				...
				\label{ex:twodisjointpaths}
			\end{ex}

			\begin{defn}
				Let $G = (V,E)$, and let $S \subseteq V, S \ne \emptyset, S \ne V$ (we pick a proper subset of vertices). The \textbf{cut induced} by $S$ is:
				\begin{align*}
	...
	\section{Trees}
		...
		\begin{proof}
			...
			\begin{enumerate}
				...
				\item If an edge is not a bridge, then that edge is in a cycle, which is a contradiction. \qedhere
			\end{enumerate}
		\end{proof}
		...
		\section{Planar Graphs}
			...
			\begin{ex}
				...
				\begin{center}
					\begin{tikzpicture}[scale=2]
						...
						\draw (0,0) to[in=-100,out=-30] (1.25,-0.25) to[in=0,out=80] (1,1);
					\end{tikzpicture}
				\end{center}
			\end{ex}
			...
			\begin{ex}
				Here are two planar embeddings of the same graph $G_2$ that look similar, except the edges are labeled differently:
				\begin{center}
					\begin{tikzpicture}[scale=2]
						\node [dot={0}{}] at (0,0) {};
						\node [dot={0}{}] at (1,1) {};
			...
			\begin{center}
				\begin{tikzpicture}[scale=1.5]
					...
					\node[draw=none] at (1.95,2.75) {$F_5$};
				\end{tikzpicture}
			\end{center}
		...
		\subsection{Review of Planar Graphs} \lecture{March 18, 2013}
			\begin{itemize}
		...
		% Eric Katz gave this lecture.
		\subsection{Non-Planar Graphs} \lecture{March 20, 2013}
			\begin{center}
				\begin{tikzpicture}[scale=1.5]
					\node [dot={0}{}] at (0,0) {};
					\node [dot={0}{}] at (2,0) {};
			...
			\\ \\
			The two following graphs are also non-planar, and are built by adding more edges and vertices to $K_5$.
			\begin{center}
				\begin{tikzpicture}[scale=1.5]
					\node [dot={0}{}] at (0,0) {};
					\node [dot={0}{}] at (2,0) {};
		...
		\subsection{Graph Colouring}
			...
			\begin{ex}
				We're going to 5-colour the following graph.
				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						\node [dot={0}{}] at (0,0) {};
						\node [dot={0}{}] at (2,0) {};
		...
		\subsection{Dual Graphs} \lecture{March 25, 2013}
			...
			\begin{ex}
				...
				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						...
						\draw (0,0) -- (3,0) -- (3,2) -- (0,2) -- (0,0);
						\draw (0,1) -- (3,1);
						\draw (0,0) .. controls (-2, 1) .. (0,2);
						\draw (3,0) .. controls (5, 1) .. (3,2);
						\draw[color=green] (-1,1) to[out=-30,in=180] (1.5,0.5) to[out=0,in=210] (4,1) to[out=150,in=0] (1.5,1.5) to[out=180,in=30] (-1, 1);
						\draw[color=green] (1.5,1.5) -- (1.5,2.5);
						\draw[color=green] (-1,1) to[out=80,in=180] (1.5,2.5) to[out=0,in=100] (4,1);
						\draw[color=green] (1.5,0.5) to[out=240,in=-90] (-2,1) to[out=90,in=180] (1.5,2.5);
					\end{tikzpicture}
				\end{center}
			...
			\begin{defn}
				Let $G = (V, E)$ be a graph with a planar embedding and faces $F_1, F_2, \ldots, F_f$. The \textbf{dual graph} $G^\star$ is the graph where:
				\begin{itemize}
					\item Vertices of $G^\star$ are $F_1, F_2, \ldots, F_f$.
					\item If $e$ is in the boundary of $F_i, F_j$ in $G$, then $e$ is joining $F_i, F_j$ in $G^\star$.
					\item If $e$ appears twice in the boundary walk $F_j$ in $G$, then $e$ is a loop of $G^\star$ with endpoints $F_j$.
				\end{itemize}
			\end{defn}
	...
	\section{Matching}
		...
		\begin{defn}
			Let $G = (V, E)$ be a graph. The \textbf{matching} $M$ of $G$ is a subset of the edges such that no two edges of $M$ share an endpoint.
		\end{defn}

		It's trivial to see that $\emptyset$ is a matching, as is any single edge in the graph.

		\begin{ex}
			The highlighted edges in the following graph form a matching because no two edges share a vertex.
			\begin{center}
				\begin{tikzpicture}[scale=1.5]
					\node [dot={0}{}] at (0,0) {};
					\node [dot={0}{}] at (4,0) {};
					...
					\draw[color=blue, ultra thick] (0,0) -- (1,1);
					\draw[color=blue, ultra thick] (0,2) -- (4,2);
					\draw[color=blue, ultra thick] (3,1) -- (4,0);
					\draw (0,0) -- (4,0) -- (4,2) -- (0,2) -- (0,0);
					\draw (0,0) -- (1,1) -- (0,2);
					\draw (4,0) -- (3,1) -- (4,2);
					\draw (1,1) -- (3,1);
		...
		\\ \\
		Ideally, the algorithm will be fast and easily verifiable. We need to be able to quickly produce a result, and we need to be able to easily convince someone that it is indeed a \emph{maximum} matching.

		\begin{defn}
			A \textbf{maximum matching} is a matching with as many edges as possible in $G$.
		\end{defn}

		\begin{defn}
			A matching is said to be \textbf{maximal} if adding any additional edge will result in a set that is no longer a matching.
		\end{defn}

		\begin{ex} \lecture{March 27, 2013}
			Find a maximum matching (a matching with as many edges as possible) for the following graph.
			\begin{center}
				\begin{tikzpicture}[scale=1.5]
					\node [dot={0}{}] at (0,1) {};
					\node [dot={0}{}] at (1,0) {};
					...
					\draw[green, ultra thick] (0,1) -- (1,2);
					\draw[green, ultra thick] (1,0) -- (3,0);
					\draw[green, ultra thick] (3,2) -- (4,1);
					\draw (0,1) -- (1,0) -- (1,2);
					\draw (3,2) -- (3,0) -- (4,1);
					\draw[red, thin] (1,0) -- (3,0);
					\draw[red, thin] (1,2) -- (3,2);
				\end{tikzpicture}
			\end{center}
		...
		\subsection{Application: co-op}
			...
			\\ \\
			Suppose $A = \set{\text{alice}, \text{bob}, \text{carol}}$ and $J = \set{1, 2, 3, 4}$. We also have that:
			\begin{itemize}
				\item Alice is qualified for jobs 1 and 3.
				\item Bob is qualified for jobs 2 and 4.
				\item Carol is qualified for jobs 1, 2, and 4.
			\end{itemize}

			We want to assign applicants to jobs such that:
			\begin{itemize}
				\item No applicant is assigned two or more jobs.
				\item No job is assigned two or more applicants.
			\end{itemize}
			...
			\begin{center}
				\begin{tikzpicture}[scale=1.5]
					...
					\draw (0,0) -- (1,0) -- (0,1) -- (1,2) -- (0,0);
					\draw (0,0) -- (1,3) -- (0,2) -- (1,1);
				\end{tikzpicture}
			\end{center}
			...
			\subsection{Saturation}
				...
				\begin{defn}
					Let $G$ be a graph with matching $M$. Vertex $v$ is \textbf{saturated} if $v$ is the endpoint of an edge of $M$. Otherwise, $v$ is said to be \textbf{unsaturated}.
				\end{defn}

				\begin{ex}
					Consider the following matching $M$, illustrated as green edges:
					\begin{center}
						\begin{tikzpicture}[scale=1.5]
							\node [dot={0}{}] at (0,0) {};
							\node [gdot={0}{}] at (0,1) {};
				...
				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						...
						\draw (4,2) -- (4,0) -- (5,1);
						\draw (2,0) -- (2,2);
						\draw[color=green,ultra thick] (0,1) -- (1,2);
						\draw[color=green,ultra thick] (2,2) -- (3,2);
						\draw[color=green,ultra thick] (3,0) -- (4,0);
						\draw[color=green,ultra thick] (4,2) -- (5,1);
						\draw[color=green,ultra thick] (1,0) -- (2,0);
			            \draw[color=red, thick] (1,0) -- (2,0);
						\draw[color=red,ultra thick] (1,2) -- (2,2);
						\draw[color=red, thick] (3,0) -- (4,0);
						\draw[color=red,ultra thick] (3,2) -- (4,2);
					\end{tikzpicture}
				\end{center}
			...
			\begin{ex}
				...
				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						...
						\draw[red, ultra thick] (1,0) -- (1,1);
						\draw[red, ultra thick] (2,0) -- (2,1);
						\draw[red, ultra thick] (3,0) -- (3,1);
						\draw (0,0) -- (3,0);
						\draw (0,1) -- (3,1);
					\end{tikzpicture}
				\end{center}
			...
			\begin{ex}
				...
				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						...
						\draw (1,0) -- (2,0);
						\draw (0,1) -- (1.5,2) -- (3,1);
						\draw[color=red, ultra thick] (0,1) -- (1,0);
						\draw[color=red, ultra thick] (2,0) -- (3,1);
					\end{tikzpicture}
				\end{center}
		...
		\subsection{XY-Structures}
			...
			\begin{ex} \lecture{April 5, 2013}
				Let's say you've already run the algorithm for a few iterations, and you have this bipartite graph:
				...
				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						...
						\node [dot={180}{4}] at (0,0) {};
						\node [dot={0}{8}] at (1,0) {};
						\node [dot={180}{3}] at (0,1) {};
						\node [dot={0}{7}] at (1,1) {};
						\node [dot={180}{2}] at (0,2) {};
						\node [dot={0}{6}] at (1,2) {};
						\node [dot={180}{1}] at (0,3) {};
						\node [dot={0}{5}] at (1,3) {};
						...
						\draw (0,1) -- (1,3);
						\draw (0,2) -- (1,3);
						\draw[color=green, ultra thick] (0,3) -- (1,3);
						\draw[color=green, ultra thick] (0,1) -- (1,0);
					\end{tikzpicture}
				\end{center}
				...
				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						...
						\node [dot={-90}{8}] at (1,0) {};
						\node [dot={-90}{3}] at (2,0) {};
						\node [dot={0}{7}] at (3, -0.5) {};
						\node [dot={0}{6}] at (3,0.5) {};
						...
						\draw (0,1) -- (1,1);
						\draw[color=green, ultra thick] (1,1) -- (2,1);
					\end{tikzpicture}
				\end{center}

				We now have that $7 \in Y$ is unsaturated. $P(7) = 48, 83, 37$. We now have $M'$: $$M' = \set{15, 38} \Delta \set{48, 38, 37} = \set{15, 48, 37}$$
				...
				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						...
						\draw[color=green, ultra thick] (0,0) -- (1,0);
						\draw[color=green, ultra thick] (0,1) -- (1,1);
						\draw (0,1) -- (1,2);
						\draw (0,1) -- (1,3);
						\draw (0,2) -- (1,3);
						\draw[color=green, ultra thick] (0,3) -- (1,3);
				...
				We have that $X = \set{1, 2}$ and $Y = \set{5}$. $Y$ contains only saturated vertices, so we stop. The cover we found is: $$C = (A \backslash X) \cup Y = \set{1, 2, 3, 4} \backslash \set{1, 2} \cup \set{5} = \set{3, 4, 5}$$

				Take the time to verify that the cover is valid.
			\end{ex}

			\begin{ex}
				Once again, suppose you have run the algorithm for a few iterations and you currently have this:

				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						\node [gdot={-90}{5}] at (0,0) {};
						\node [rdot={-90}{2}] at (1,0) {};
						\node [gdot={-90}{7}] at (2,0) {};
						\node [rdot={-90}{4}] at (3,0) {};

						\node [rdot={90}{1}] at (0,1) {};
						\node [gdot={90}{6}] at (1,1) {};
						\node [rdot={90}{3}] at (2,1) {};
						\node [gdot={90}{8}] at (3,1) {};

						\draw (0,1) -- (0,0) -- (1,0) -- (1,1) -- (2,1) -- (2,0) -- (3,0) -- (3,1);
						\draw[color=blue, ultra thick] (0,1) -- (1,1);
						\draw[color=blue, ultra thick] (1,0) -- (2,0);
						\draw[color=blue, ultra thick] (2,1) -- (3,1);
					\end{tikzpicture}
				\end{center}

				This is a bipartite graph with partitions $A = \set{1, 2, 3, 4}$ (red) and $B = \set{5, 6, 7, 8}$ (green).
				\\ \\
				...
				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						...
						\draw (0,1) -- (1,2);
						\draw[color=blue, ultra thick] (1,2) -- (2,2);
						\draw (2,2) -- (3,2);
						\draw[color=blue, ultra thick] (3,2) -- (4,2);
					\end{tikzpicture}
				\end{center}

				We now have $5 \in Y$ which is an unsaturated vertex. $P(5) = 47, 72, 25$. We now have $M'$: $$M' = M \Delta P(5) = \set{16, 38, 27} \backslash \set{47, 72, 25} = \set{16, 38, 47, 25}$$
				...
				We now have that $X = Y = \emptyset$. There is no vertex in $A$ that is unsaturated, so we should be done. The cover we found is: $$C = (A \backslash X) \cup Y = A = \set{1, 2, 3, 4}$$
			\end{ex}
			...
			\begin{ex}
				...
				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						...
						\node [rdot={180}{3}] at (0,0) {};
						\node [gdot={0}{6}] at (1,0) {};
						\node [rdot={180}{2}] at (0,1) {};
						\node [gdot={0}{5}] at (1,1) {};
						\node [rdot={180}{1}] at (0,2) {};
						\node [gdot={0}{4}] at (1,2) {};

						\draw (0,0) -- (1,0) -- (0,1) -- (1,2) -- (0,2) -- (1,1);
					\end{tikzpicture}
				\end{center}
	...
	\section*{Clicker Questions}
		\begin{itemize}
			...
			\item What is the minimum number of colours needed to colour this graph (not shown here)? Four, because this was contained within the graph:
				\begin{center}
					\begin{tikzpicture}[scale=2]
						\node [dot={0}{}] at (0,0) {};
						\node [dot={0}{}] at (1,1) {};
			...
			\item Do the following embeddings have the same duals?
				\begin{center}
					\begin{tikzpicture}[scale=2.5]
						...
						\node [dot={0}{}] at (0.75,0.75) {};

						\draw (0,0) -- node [midway,fill=white] {1} (0,1) -- node [midway,fill=white] {2} (1.75, 1.5) -- node [midway,fill=white] {4}  (0.75,0.75) -- node [midway,fill=white] {7} (1,0) -- node [midway,fill=white] {6} (0,0);
						\draw (1,0) -- node [midway,fill=white] {3} (0,1);
						\draw (1,0) -- node [midway,fill=white] {5} (1.75,1.5);
					\end{tikzpicture}
					\begin{tikzpicture}[scale=2.5]
						\node [dot={0}{}] at (0,0) {};
						\node [dot={0}{}] at (0,1) {};
						...
						\draw (0,0) -- node [midway,fill=white] {1} (0,1) -- node [midway,fill=white] {2} (1.75, 1.5) -- node [midway,fill=white] {4}  (2,0.33) -- node [midway,fill=white] {7} (1,0) -- node [midway,fill=white] {6} (0,0);
						\draw (1,0) -- node [midway,fill=white] {3} (0,1);
						\draw (1,0) -- node [midway,fill=white] {5} (1.75,1.5);
					\end{tikzpicture}
				\end{center}

				No. There are no boundary walks of length five in the first graph, but there are in the second. The planar duals are not the same. Note that the dual is of the \emph{drawing}, not of the graph itself.

			\item How many faces does a connected planar graph with 30 vertices and 100 edges have? By Euler's formula, $p - q + f 2$, we get that $f = 2 - p + q = 2 - 30 + 100 = 72$.

			\item What is the minimum number of edges you need to remove from this graph to make it planar?
			...
			\begin{center}
				\begin{tikzpicture}[scale=3]
					...
					\draw (0,1) -- (1,0) -- (2,1) -- (0.75,0.5) -- (1.25,0.5) -- (1,0.9) -- (0.75,0.5);
					\draw (2,1) -- (0,1) -- (1.25,0.5);
					\draw (1,0) -- (1,0.9);
					\draw (1,0) -- (0.75,0.5);
					\draw (1,0) -- (1.25,0.5);
					\draw (0,1) -- (0.75,0.5);
			...
			\item Do the red edges in the following form a matching?
			...
			\begin{center}
				\begin{tikzpicture}[scale=2]
					...
					\draw[color=red, ultra thick] (0,0) -- (0,1);
					\draw[color=red, ultra thick] (0,2) -- (1,1);
					\draw[color=red, ultra thick] (2,1.5) -- (2,0);
					\draw[color=red, ultra thick] (4,0) -- (3,0.75);
					\draw (0,1) -- (0,2) -- (4,2) -- (4,0) -- (0,0);
					\draw (0,0) -- (1,1) -- (0,1);
					\draw (1,0) -- (1,1) -- (2,1.5) -- (4,2) -- (3,0.75) -- (2,0);
					\draw (0,2) -- (2,1.5);
				\end{tikzpicture}
			\end{center}
